...
@Component({
  selector: 'ngw-liquid-ether',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './liquid-ether.component.html',
  styleUrl: './liquid-ether.component.css'
})
export class LiquidEtherComponent implements AfterViewInit, OnDestroy, OnChanges {
  ...
  ngAfterViewInit(): void {
    ...
    this.initLiquidEther();
  }

  ngOnChanges(changes: SimpleChanges): void {
    if (!isPlatformBrowser(this.platformId) || !this.webgl) {
      return;
    }
    ...
    if (changes['colors']) {
      const paletteTex = this.makePaletteTexture(this.colors);
      if (this.webgl.output?.output?.material && 'uniforms' in this.webgl.output.output.material) {
        (this.webgl.output.output.material as THREE.RawShaderMaterial).uniforms['palette'].value = paletteTex;
      }
    }
  }
...
class MouseClass {
  ...
  private _onMouseMove = (event: MouseEvent) => this.onDocumentMouseMove(event);
  private _onTouchStart = (event: TouchEvent) => this.onDocumentTouchStart(event);
  private _onTouchMove = (event: TouchEvent) => this.onDocumentTouchMove(event);
  private _onTouchEnd = () => this.onTouchEnd();
  private _onDocumentLeave = () => this.onDocumentLeave();
...
class Advection extends ShaderPass {
  ...
  updateAdvection(props: { dt: number; isBounce: boolean; BFECC: boolean }): void {
...
class ExternalForce extends ShaderPass {
  ...
  private simProps: any;

  constructor(simProps: any) {
    super({ output: simProps.dst });
    this.simProps = simProps;
    this.initExternalForce();
  }

  initExternalForce(): void {
    super.init();
  ...
  updateExternalForce(props: {
    mouse_force: number;
    cursor_size: number;
    cellScale: THREE.Vector2;
  }): void {
    if (!this.mouse || !('uniforms' in this.mouse.material)) return;
...
class Viscous extends ShaderPass {
  ...
  updateViscous(props: { viscous: number; iterations: number; dt: number }): THREE.WebGLRenderTarget {
...
class Divergence extends ShaderPass {
  ...
  updateDivergence(props: { vel: THREE.WebGLRenderTarget }): void {
...
class Poisson extends ShaderPass {
  ...
  updatePoisson(props: { iterations: number }): THREE.WebGLRenderTarget {
...
class Pressure extends ShaderPass {
  ...
  updatePressure(props: { vel: THREE.WebGLRenderTarget; pressure: THREE.WebGLRenderTarget }): void {
    this.uniforms.velocity.value = props.vel.texture;
...
class Simulation {
  ...
  createAllFBO(): void {
    ...
    for (let key in this.fbos) {
      this.fbos[key as keyof typeof this.fbos] = new THREE.WebGLRenderTarget(this.fboSize.x, this.fboSize.y, opts);
    }
  }

  createShaderPass(): void {
  ...
  resize(): void {
    this.calcSize();
    for (let key in this.fbos) {
      const fbo = this.fbos[key as keyof typeof this.fbos];
...
class Output {
  ...
  init(paletteTex: THREE.DataTexture, bgVec4: THREE.Vector4): void {
...
class WebGLManager {
  ...
  output!: Output;
  running = false;
  private _loop = () => this.loop();
  private _resize = () => this.resize();
  ...
  init(): void {
    ...
    this.output.init(this.props.paletteTex, this.props.bgVec4);
  }
  ...
  dispose(): void {
    try {
      ...
      if (Common.renderer) {
        ...
        // Force lose WebGL context if supported
        try {
          ...
          // Extension not supported, continue with normal disposal
        }
        Common.renderer.dispose();
        // forceContextLoss may not exist in all Three.js versions
        if (typeof Common.renderer.forceContextLoss === 'function') {

...
class Geometry {
  ...
  get data(): { vertices: Float32Array; indices: Uint16Array; normals: Float32Array; uvs: Float32Array } {
    return {
...
function makeBuffer(gl: WebGL2RenderingContext, sizeOrData: number | ArrayBufferView, usage: number): WebGLBuffer | null {
...
class ArcballControl {
  ...
  quatFromVectors(a: vec3, b: vec3, out: quat, angleFactor = 1): { q: quat; axis: vec3; angle: number } {
...
class InfiniteGridMenu {
  ...
  private discLocations: Record<string, WebGLUniformLocation | null> = {};
  ...
  constructor(
    ...
    onInit?: (instance: InfiniteGridMenu) => void
  ) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  private init(onInit?: (instance: InfiniteGridMenu) => void): void {
    ...
    } as Record<string, number | WebGLUniformLocation | null>;
    ...
    const aModelPositionLoc = this.discLocations['aModelPosition'] as number;
    const aModelUvsLoc = this.discLocations['aModelUvs'] as number;
    this.discVAO = makeVertexArray(
      gl,
      [
        [vertexBuffer, aModelPositionLoc, 3],
        [uvBuffer, aModelUvsLoc, 2]
      ],
  ...
  private initDiscInstances(count: number): void {
    ...
    if (!this.discVAO || !this.discInstances.buffer) return;
    gl.bindVertexArray(this.discVAO);
    gl.bindBuffer(gl.ARRAY_BUFFER, this.discInstances.buffer);
    gl.bufferData(gl.ARRAY_BUFFER, this.discInstances.matricesArray.byteLength, gl.DYNAMIC_DRAW);
    const mat4AttribSlotCount = 4;
    const bytesPerMatrix = 16 * 4;
    const aInstanceMatrixLoc = (this.discLocations['aInstanceMatrix'] as number) || 0;
  ...
  private render(): void {
    const gl = this.gl;
    if (!this.discProgram || !this.discVAO) return;
    ...
    const uWorldMatrix = this.discLocations['uWorldMatrix'] as WebGLUniformLocation | null;
    const uViewMatrix = this.discLocations['uViewMatrix'] as WebGLUniformLocation | null;
    const uProjectionMatrix = this.discLocations['uProjectionMatrix'] as WebGLUniformLocation | null;
    const uCameraPosition = this.discLocations['uCameraPosition'] as WebGLUniformLocation | null;
    const uRotationAxisVelocity = this.discLocations['uRotationAxisVelocity'] as WebGLUniformLocation | null;
    const uItemCount = this.discLocations['uItemCount'] as WebGLUniformLocation | null;
    const uAtlasSize = this.discLocations['uAtlasSize'] as WebGLUniformLocation | null;
    const uFrames = this.discLocations['uFrames'] as WebGLUniformLocation | null;
    const uScaleFactor = this.discLocations['uScaleFactor'] as WebGLUniformLocation | null;
    const uTex = this.discLocations['uTex'] as WebGLUniformLocation | null;

    if (uWorldMatrix) {
      gl.uniformMatrix4fv(uWorldMatrix, false, this.worldMatrix);
    }
    if (uViewMatrix) {
      gl.uniformMatrix4fv(uViewMatrix, false, this.camera.matrices.view);
    }
    if (uProjectionMatrix) {
      gl.uniformMatrix4fv(uProjectionMatrix, false, this.camera.matrices.projection);
    }
    if (uCameraPosition) {
      gl.uniform3f(
        uCameraPosition,
    ...
    if (uItemCount) {
      gl.uniform1i(uItemCount, this.items.length);
    }
    if (uAtlasSize) {
      gl.uniform1i(uAtlasSize, this.atlasSize);
    }

    if (uFrames) {
      gl.uniform1f(uFrames, this.frames);
    }
    if (uScaleFactor) {
      gl.uniform1f(uScaleFactor, this.scaleFactor);
    }
    if (uTex) {
      gl.uniform1i(uTex, 0);
    }
...
@Component({
  selector: 'ngw-infinite-menu',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './infinite-menu.component.html',
  styleUrl: './infinite-menu.component.css'
})
export class InfiniteMenuComponent implements AfterViewInit, OnDestroy, OnChanges {
  ...
  private sketch?: InfiniteGridMenu;
  private resizeListener?: () => void;
  private activeItem = signal<MenuItem | null>(null);
  private isMoving = signal(false);

  readonly activeItemSignal = signal<MenuItem | null>(null);
  readonly isMovingSignal = signal(false);
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (!isPlatformBrowser(this.platformId) || !this.sketch) {
      return;
    }
  ...
  private initMenu(): void {
    ...
    const itemsToUse = this.items.length ? this.items : defaultItems;
    this.sketch = new InfiniteGridMenu(
      canvas,
      itemsToUse,
      handleActiveItem,
      (isMoving: boolean) => {
        this.isMoving.set(isMoving);
        this.isMovingSignal.set(isMoving);
      },
      sk => sk.run()
    );
  }
